...
import com.dfg.java_template.framework.pay.aliyun.service.AliPayAPPMent;
import com.dfg.java_template.framework.pay.aliyun.vo.*;
import com.dfg.java_template.framework.pay.aliyun.vo.app.AliPayTradeAppPayVO;
...
@Slf4j
public class AliPayAPPService implements AliPayAPPMent {
    @Resource
    private AlipayConfigs aliPayConfigs;
    
    @Override
    public String pay(AliPayTradeAppPayVO aliPayTradeAppPayVO) throws AlipayApiException {
        AlipayClient aliPayClient = new DefaultAlipayClient(aliPayConfigs.getAlipayConfig());
        // 设置支付请求模型
        AlipayTradeAppPayModel model=new AlipayTradeAppPayModel();
        model.setOutTradeNo(aliPayTradeAppPayVO.getOutTradeNo());    // 商户订单号 必选
        model.setTotalAmount(String.valueOf(aliPayTradeAppPayVO.getTotalAmount()));    // 订单金额 必选
        model.setSubject(aliPayTradeAppPayVO.getSubject());   // 订单标题 必选
