...
import com.dfg.java_template.framework.pay.aliyun.service.AliPayWAPMent;
...
@Slf4j
public class AliPayWAPService implements AliPayWAPMent {
    @Resource
    private AlipayConfigs aliPayConfigs;
    
    @Override
    public String pay(AliPayTradeWapVO aliPayTradeWapVO) throws AlipayApiException {
        AlipayClient aliPayClient = new DefaultAlipayClient(aliPayConfigs.getAlipayConfig());
        // 设置支付请求模型
        AlipayTradeWapPayModel model=new AlipayTradeWapPayModel();
        model.setOutTradeNo(aliPayTradeWapVO.getOutTradeNo());    // 商户订单号 必选
        model.setTotalAmount(String.valueOf(aliPayTradeWapVO.getTotalAmount()));    // 订单金额 必选
        model.setSubject(aliPayTradeWapVO.getSubject());   // 订单标题 必选
